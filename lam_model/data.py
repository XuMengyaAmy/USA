    ...
    def __getitem__(self, idx):
        ...
        for attempt in range(max_attempts):
            try:
                ...
                # Use the global preprocess function instead of self.preprocess
                resolution = self.resolution
                class_name = get_parent_dir(self._clips.video_paths[video_idx])
                label = self.class_to_label[class_name]
                
                # Process video using the global preprocess function
                processed_video = preprocess(video, resolution)
                return {'video': processed_video, 'label': label}
                ...
                if attempt == max_attempts - 1:
                    # If we've tried max_attempts times, return a dummy video
                    print(f"Failed to load video after {max_attempts} attempts. Returning dummy video.")
                    # Create a dummy video tensor with the right shape
                    dummy_video = torch.zeros((3, self.sequence_length, self.resolution, self.resolution), 
                                             dtype=torch.float32)
                    return {'video': dummy_video, 'label': 0}  # Use 0 as a default label
