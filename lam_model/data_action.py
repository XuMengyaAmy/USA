...
class VideoDataset(data.Dataset):
    ...
    def __init__(self, data_folder, sequence_length, train=True, resolution=64):
        ...
        folder = osp.join(data_folder, 'train' if train else 'val')
    ...
    def __getitem__(self, idx):
        ...
        for attempt in range(max_attempts):
            try:
                video, _, _, video_idx = self._clips.get_clip(current_idx)
                
                resolution = self.resolution
                class_name = get_parent_dir(self._clips.video_paths[video_idx])
                label = self.class_to_label[class_name]
                
                # Get clip name from the video path
                clip_path = self._clips.video_paths[video_idx]
                clip_name = osp.basename(clip_path)
                # Remove file extension if present
                clip_name = osp.splitext(clip_name)[0]
                
                # Process video
                processed_video = preprocess(video, resolution)  # [C, T, H, W]
                
                # Split into initial frame and action sequence
                initial_frame = processed_video[:, 0]  # [C, H, W]
                action_frames = processed_video[:, 1:]  # [C, T-1, H, W]
                
                return {
                    'video': processed_video,  # Full sequence for reconstruction loss
                    'initial_frame': initial_frame,  # First frame
                    'action': action_frames,  # Remaining frames as action sequence
                    'label': label,
                    'clip_name': clip_name  # Add the clip name
                }
                
            except Exception as e:
                print(f"Error loading video at index {current_idx}: {str(e)}")
                current_idx = (current_idx + 1) % len(self._clips)
                
                if attempt == max_attempts - 1:
                    print(f"Failed to load video after {max_attempts} attempts. Returning dummy data.")
                    dummy_video = torch.zeros((3, self.sequence_length, self.resolution, self.resolution))
                    return {
                        'video': dummy_video,
                        'initial_frame': dummy_video[:, 0],
                        'action': dummy_video[:, 1:],
                        'label': 0,
                        'clip_name': f"dummy_clip_{current_idx}"  # Add dummy clip name
                    }
...
class HDF5Dataset(data.Dataset):
    ...
    def __init__(self, data_file, sequence_length, train=True, resolution=64):
        ...
        # Try to get clip names if they exist in the HDF5 file
        self._clip_names = None
        if f'{self.prefix}_clip_names' in self.data:
            self._clip_names = self.data[f'{self.prefix}_clip_names']
    ...
    def __getitem__(self, idx):
        ...
        processed_video = preprocess(video, self.resolution)  # [C, T, H, W]
        
        # Get clip name if available, otherwise use index
        if self._clip_names is not None:
            clip_name = self._clip_names[idx]
            # Convert bytes to string if necessary
            if isinstance(clip_name, bytes):
                clip_name = clip_name.decode('utf-8')
        else:
            # Use formatted index as the clip name
            clip_name = f"clip_{idx:04d}"
        
        return {
            'video': processed_video,  # Full sequence
            'initial_frame': processed_video[:, 0],  # First frame
            'action': processed_video[:, 1:],  # Remaining frames
            'clip_name': clip_name
        }
